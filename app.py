...
try:
  ...
  def analyze_data_quality(df):
    return {
        ...
        'is_perfect_quality': (
            df.isnull().sum().sum() == 0 and df.duplicated().sum() == 0
        ),
    ...
    return [
        {
            ...
            'question': 'Which categories drive the highest metric totals?',
            'purpose': 'Identify core operational drivers.',
        },
        {
            'category': 'Risk',
            'question': (
                'Are there extreme statistical anomalies in numerical metrics?'
            ),
            'purpose': 'Mitigate operational risks.',
        },
        {
            'category': 'Trend',
            'question': 'What is the continuous metric trajectory over time?',
            'purpose': 'Evaluate performance stability.',
        },
    ]
...
def get_analytical_columns(df):
  ...
  clean_cat_cols = [
      c
      for c in cat_cols
      if not (c.lower().endswith('id') or c.lower().startswith('id'))
      and df[c].nunique() <= 50
  ]
...
def load_data(uploaded_file, row_limit=5000):
  try:
    ...
    if len(df) > row_limit:
      st.warning(
          f'⚠️ Dataset contains {len(df):,} records. Sampled down to'
          f' {row_limit:,} rows for performance optimization.'
      )
...
def train_risk_model(df, target_col):
  # Drop high cardinality / non-analytical ID columns prior to dropping nulls
  drop_cols = [
      c
      for c in df.columns
      if c != target_col
      and (c.lower().endswith('id') or df[c].nunique() > 100)
  ]
  data = df.drop(columns=drop_cols).dropna()

  if data.empty:
    raise ValueError(
        'Dataset has no complete rows after dropping null values.'
    )
  ...
  if X.empty or X.shape[1] == 0:
    raise ValueError(
        'Insufficient analytical features remaining to train model.'
    )
...
def generate_native_pdf_report(df, domain_info, quality_info, recommendations):
  ...
  story.append(
      Paragraph('Data Analyzer AI — Executive Analytics Report', title_style)
  )
  story.append(Spacer(1, 4))
  domain_text = (
      f"<b>Detected Domain:</b> {domain_info.get('domain', 'General')} |"
      f" <b>Confidence:</b> {domain_info.get('confidence', 'Low')}"
  )
  ...
  if num_cols:
    try:
      ...
      if cat_cols:
        ...
        top_cats.plot(
            kind='bar', ax=axs[0, 0], color='#2563EB', edgecolor='none'
        )
        axs[0, 0].set_title(
            f'1. Total {num_cols[0]} by {cat_cols[0]}',
            fontsize=8.5,
            fontweight='bold',
            pad=6,
        )
        ...
        axs[0, 0].hist(
            df[num_cols[0]].dropna(),
            bins=15,
            color='#2563EB',
            edgecolor='white',
        )
        axs[0, 0].set_title(
            f'1. Distribution of {num_cols[0]}',
            fontsize=8.5,
            fontweight='bold',
            pad=6,
        )
      ...
      if cat_cols:
        avg_data = (
            df.groupby(cat_cols[0])[num_cols[0]].mean().nlargest(6).sort_values()
        )
        avg_data.plot(kind='barh', ax=axs[1, 0], color='#1D4ED8')
        axs[1, 0].set_title(
            f'3. Avg {num_cols[0]} per {cat_cols[0]}',
            fontsize=8.5,
            fontweight='bold',
            pad=6,
        )
      ...
      if cat_cols:
        ...
        vol_data.plot(
            kind='bar', ax=axs[1, 1], color='#60A5FA', edgecolor='none'
        )
        axs[1, 1].set_title(
            f'4. Record Volume by {cat_cols[0]}',
            fontsize=8.5,
            fontweight='bold',
            pad=6,
        )
      ...
      story.append(
          Paragraph(f'<i>Visual Rendering Exception: {err}</i>', body_style)
      )
  ...
  if recommendations:
    for idx, rec in enumerate(recommendations[:3], 1):
      story.append(
          Paragraph(
              f"<b>Rec {idx}: {rec.get('business_area', 'Strategy')}</b>",
              bold_style,
          )
      )
      story.append(
          Paragraph(
              f"• <b>Action:</b> {rec.get('action_development', 'N/A')}",
              body_style,
          )
      )
    ...
    story.append(
        Paragraph(
            '<i>No automated recommendations generated for this dataset.</i>',
            body_style,
        )
    )
...
if uploaded_file is not None:
  ...
  if df is not None and not df.empty:
    ...
    target_field = st.sidebar.selectbox(
        'Select Target / Driver Field:', df.columns
    )
    cmap_choice = st.sidebar.selectbox(
        'Matplotlib Color Map:',
        ['Blues', 'viridis', 'plasma', 'cividis', 'coolwarm', 'YlGnBu'],
        index=0,
    )
    ...
    try:
      recommendations = generate_recommendations(
          df, quality_info, relationships, domain_info
      )
    except Exception:
      recommendations = []
    ...
    st.sidebar.markdown('---')
    pdf_bytes = generate_native_pdf_report(
        df, domain_info, quality_info, recommendations
    )
    ...
    st.caption(
        'Intelligent Visual Analytics & Reporting Platform (Matplotlib /'
        ' Seaborn)'
    )
    st.info(
        f"Detected Industry / Domain: **{domain_info.get('domain', 'General')}**"
        f" (Confidence: **{domain_info.get('confidence', 'Low')}**)"
    )
    ...
    with tab1:
      ...
      c1.metric(
          'Loaded Records', f"{quality_info.get('total_rows', len(df)):,}"
      )
      c2.metric(
          'Total Columns', f"{quality_info.get('total_cols', len(df.columns)):,}"
      )
    ...
    with tab2:
      st.subheader(
          'Matplotlib Dashboard Analytics (4 Figures Per Dashboard)'
      )
      ...
      with db_tab1:
        ...
        if cat_cols and num_cols:
          ...
          c_dim = col_a.selectbox(
              'Categorical Dimension:', cat_cols, index=0, key='db1_cat'
          )
          m_metric = col_b.selectbox(
              'Numerical Metric:', num_cols, index=0, key='db1_num'
          )
          ...
          axs[0, 0].set_title(
              f'1. Total {m_metric} by {c_dim}', fontweight='bold'
          )
          ...
          axs[0, 1].pie(
              ...
              autopct='%1.1f%%',
              startangle=140,
              colors=sns.color_palette(cmap_choice, len(pie_data)),
          )
          axs[0, 1].set_title(
              f'2. {c_dim} Frequency Share', fontweight='bold'
          )

          # 3. Horizontal Average Bar
          agg_avg = (
              df.groupby(c_dim)[m_metric].mean().nlargest(8).sort_values()
          )
          sns.barplot(
              x=agg_avg.values,
              y=agg_avg.index,
          ...
          axs[1, 0].set_title(
              f'3. Avg {m_metric} by {c_dim}', fontweight='bold'
          )
          ...
          axs[1, 1].set_title(
              f'4. Record Count per {c_dim}', fontweight='bold'
          )
          ...
          st.warning(
              'Requires categorical and numerical fields to display Dashboard'
              ' 1.'
          )
      ...
      with db_tab2:
        ...
        if len(num_cols) >= 2:
          col_a, col_b = st.columns(2)
          x_met = col_a.selectbox(
              'X-Axis Metric:', num_cols, index=0, key='db2_x'
          )
          y_met = col_b.selectbox(
              'Y-Axis Metric:',
              num_cols,
          ...
          axs[0, 0].set_title(
              f'1. Scatter ({x_met} vs {y_met})', fontweight='bold'
          )
          ...
          axs[0, 1].set_title(
              f'2. Density Kernel ({x_met} vs {y_met})', fontweight='bold'
          )

          # 3. Profile Line Chart
          df_sorted = df.sort_values(by=x_met).head(150)
          axs[1, 0].plot(
              df_sorted[x_met].values,
              df_sorted[y_met].values,
              color='#2563EB',
              linewidth=1.5,
          )
          axs[1, 0].set_title(
              f'3. Profile Trajectory ({x_met} vs {y_met})', fontweight='bold'
          )
          ...
          st.warning(
              'Dashboard 2 requires at least two numerical metrics.'
          )

      # DASHBOARD 3
      with db_tab3:
        st.markdown(
            '#### Dashboard 3: Statistical Distributions & Time Trends'
        )
        if num_cols:
          col_a, col_b = st.columns(2)
          dist_col = col_a.selectbox(
              'Metric for Distribution:', num_cols, index=0, key='db3_dist'
          )
          group_cat = col_b.selectbox(
              'Category Dimension Split:',
              [None] + cat_cols,
              index=0,
              key='db3_split',
          )
          ...
          sns.boxplot(
              ...
              y=dist_col,
              ax=axs[0, 0],
              palette=cmap_choice if group_cat else None,
          )
          axs[0, 0].set_title(
              f"1. Boxplot Spread for '{dist_col}'", fontweight='bold'
          )
          if group_cat:
            axs[0, 0].tick_params(axis='x', rotation=30)
          ...
          axs[0, 1].set_title(
              f"2. Violin Density for '{dist_col}'", fontweight='bold'
          )
          if group_cat:
          ...
          sns.histplot(
              ...
              kde=True,
              ax=axs[1, 0],
              palette='Set2' if group_cat else None,
          )
          axs[1, 0].set_title(
              f"3. Histogram & KDE of '{dist_col}'", fontweight='bold'
          )
          ...
          if date_cols:
            ...
            axs[1, 1].set_title(
                f"4. Average '{dist_col}' Timeline", fontweight='bold'
            )
            axs[1, 1].tick_params(axis='x', rotation=30)
          else:
            sns.ecdfplot(data=df, x=dist_col, hue=group_cat, ax=axs[1, 1])
            axs[1, 1].set_title(
                f"4. ECDF Cumulative Spread for '{dist_col}'", fontweight='bold'
            )
      ...
      with db_tab4:
        ...
        if questions:
          ...
          selected_q = st.selectbox(
              ...
              ['-- Select or type below --'] + suggested_q,
          )
        else:
        ...
        if st.button('🔎 Render Dashboard 4', type='primary'):
          if custom_q.strip():
            matched_num = [
                c for c in num_cols if c.lower() in custom_q.lower()
            ]
            matched_cat = [
                c for c in cat_cols if c.lower() in custom_q.lower()
            ]
            ...
            if c_col:
              ...
              axs[0, 0].set_title(
                  f'1. Total {m_col} by {c_col}', fontweight='bold'
              )
              axs[0, 0].tick_params(axis='x', rotation=30)
              ...
              axs[0, 1].set_title(
                  f'2. Avg {m_col} by {c_col}', fontweight='bold'
              )
              axs[0, 1].tick_params(axis='x', rotation=30)
              ...
              axs[1, 0].set_title(
                  f'3. {c_col} Proportion', fontweight='bold'
              )
            else:
              sns.histplot(df[m_col], ax=axs[0, 0], kde=True)
              axs[0, 0].set_title(
                  f'1. Histogram of {m_col}', fontweight='bold'
              )
              ...
              axs[1, 0].set_title(
                  f'3. Sequential Trend of {m_col}', fontweight='bold'
              )

            if len(num_cols) >= 2:
              second_m = (
                  num_cols[1] if num_cols[1] != m_col else num_cols[0]
              )
              sns.scatterplot(
                  data=df, x=m_col, y=second_m, ax=axs[1, 1]
              )
              axs[1, 1].set_title(
                  f'4. {m_col} vs {second_m}', fontweight='bold'
              )
            else:
    ...
    with tab3:
      ...
      if quality_info.get('is_perfect_quality', False):
        st.success(
            '✅ Dataset is completely clean with zero missing values or'
            ' duplicate rows.'
        )
      else:
        st.warning(
            '⚠️ Data Quality Notice: Missing values or duplicate rows detected.'
        )
      ...
      with col_q2:
        ...
        st.write(
            f"- Duplicate Records: **{quality_info.get('duplicate_rows', 0)}**"
        )
        st.write(
            '- Empty Columns:'
            f" **{len(quality_info.get('empty_cols', []))}**"
        )
        st.write(
            '- Columns with Outliers:'
            f" **{len(quality_info.get('outliers', {}))}**"
        )
    ...
    with tab5:
      ...
      if questions:
        for q in questions:
          with st.expander(
              f"📌 {q.get('category', 'Analysis')}: {q.get('question', '')}"
          ):
    ...
    with tab6:
      ...
      st.write(
          'Train a Random Forest model on demand to uncover primary metric'
          ' impact drivers.'
      )

      if st.button(
          '🚀 Train & Calculate Feature Importances', type='primary'
      ):
        with st.spinner('Training Random Forest model...'):
          try:
            model, encoders, importances, feature_names = train_risk_model(
                df, target_field
            )
            ...
            with m_col1:
              ...
              ax.set_title(
                  f"Feature Importances Driving '{target_field}'",
                  fontweight='bold',
              )
              st.pyplot(fig)
              plt.close(fig)
  ...
  st.info(
      '👆 Upload a CSV or Excel dataset from the sidebar to launch the analytics'
      ' suite.'
  )

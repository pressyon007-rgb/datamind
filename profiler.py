...
def get_data_profile(df):
    ...
    profile["Missing %"] = (
        profile["Missing"] / max(len(df), 1) * 100
    ).round(2)
...
def get_missing_values(df):
    ...
    result["Missing %"] = (
        result["Missing Count"] / max(len(df), 1) * 100
    ).round(2)
...
def get_data_quality_score(df):
    ...
    duplicate_penalty = (duplicate_rows / max(len(df), 1) * 100)
...
def get_summary_statistics(df):
    ...
    result["missing_%"] = (numeric_df.isna().mean() * 100).round(2)
...
def detect_outliers(df):
    ...
    for column in numeric_columns:
        ...
        results.append({
            "Column": column,
            "Outliers": int(count),
            "Outlier %": round(count / max(len(series), 1) * 100, 2)
        })
